...
# ──────────────────────────────────────────────────────────────
# 1. 时移规则配置（半开区间 [start, end)）
# ──────────────────────────────────────────────────────────────
LAG_RULES = [
    # (起始时刻,              结束时刻（含），        NARI滞后分钟数)
    # 数据粒度为 1 分钟，时间戳均为整分钟，区间完全覆盖所有采样点。
...
def assign_lag_column(df: pd.DataFrame) -> pd.Series:
    """向量化地为每行分配 lag 值（比逐行 apply 快 10 倍以上）。"""
...
def align(input_path: str, output_path: str) -> pd.DataFrame:
    ...
    （即 NARI 时间戳前移 lag 分钟后与 SCADA 时间对齐）。
    ...
    # ── 3. 构建 NARI 时间索引，便于快速查找 ───────────────────────
    print("[3/4] 对齐 NARI 数据到 SCADA 时间轴...")
    # 以时间为索引的 NARI 查找表
    nari_lookup = df_raw.set_index("time")[["nari_power", "nari_wind"]]

    # 向量化处理：按 lag 分组，批量做时间偏移后 merge
    aligned_parts = []
    for lag_val, group in df_raw.groupby("lag_min"):
        ...
        nari_shifted = (
            ...
            .rename(columns={"nari_power": "ACTIVE_POWER_#56_对齐",
...
def main():
    ...
    default_input  = os.path.join(repo_root, "DATA", "峡沙56号_合并风机导出数据和南瑞数据.csv")
    default_output = os.path.join(repo_root, "DATA", "峡沙56号_时间戳对齐后数据.csv")
